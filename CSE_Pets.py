...
def week_order(days_order, unordered_days, pets_per_day):
    ...
    """    
    # Maps days to their respective counts 
    mapped = {}
    for i in range(len(unordered_days)):
        mapped[unordered_days[i]] = pets_per_day[i] 
     
    # Reorders mapped dictionary to correct order of days in a week
    ordered_count = []
...
def write_descriptions_csv(input_file, output_csv):  
    """
    Takes in an input file and a an output csv file name.
    Creates a new csv file in the current directory with certain columns. 
    Return None 
    """  
...
def filter_dataframe(filename, col_names, record):
    ...
    selected = df[col_names] 
    filtered = selected[selected.Record_Type == record]
    
    return filtered    

def group_variable_count(df, col_name):
    """
    Takes in a dataframe and a column name.
    
    Returns a dataframe grouped with respect to the entries in column and counts
    of each entry per group. 
    """
    group = df.groupby(col_name)
    count = (group.agg({"Record_Type": "count"}) 
                  .rename(columns={"Record_Type": "Count"})
            )
...
def wordcloud(filename, mask_file, output_name):
    ...
    # Sets shape of word cloud to given image
...
def main(): 
    """
    Main function, executed when Final_Project.py is run as a Python script.
    """
    ...
    # Transforming date info into day of week and creating a new column for
    # day of week
    filter_days['Date'] = pd.to_datetime(filter_days['Date']) 
    filter_days['day_of_week'] = filter_days['Date'].dt.dayofweek 
    days = {0:'Monday',1:'Tuesday',2:'Wednesday',3:'Thursday',4:'Friday',
            5:'Saturday',6:'Sunday'} 
    filter_days['day_of_week'] = filter_days['day_of_week'].apply(lambda x: days[x]) 
    
    day_counts = group_variable_count(filter_days, "day_of_week")
    ...
    plot_scatter(days.values(), ordered_counts, "Days of the week", "Pet counts", \
    ...
    filter_adoptable = filter_dataframe("lost__found__adoptable_pets.csv", \
                                     ...
                                     "ADOPTABLE")
    # Grouping 
    location_count = group_variable_count(filter_adoptable, "Current_Location")              
